...
Fenetre::Fenetre()
{
    ...
    std::vector<std::string> lstconfgtmp;
    if( CfgMTStartApp.ListerLesBlocs( &lstconfgtmp ) )
		session.MiseAJourListeConfig(lstconfgtmp);
/**
...
/******/
	...
	if(!session.oksession()) { MessageInfo(session.LibelleErreur()); }
	ChargerUnContexte();
	show();
}
Fenetre::~Fenetre()
...
/** 	SLOTS MENU Gestion ( barre à droite ) **/
void Fenetre::LancerContexteEnCours()
{
    ...
    if(demarreur.DesProcessusSontDemarres())
    {
		if(plstchoixdialogue!=nullptr) ViderListeChoix();
			...
			plstchoixdialogue->push_back("démarrer sans les stopper.");
			plstchoixdialogue->push_back("les stopper avant démarrage.");
			plstchoixdialogue->push_back("abandonner le démarrage.");
				pFncRetourValeur = &Fenetre::reponse_choix_demarrer; 
				pFncAppeloui = nullptr; 
				pFncAppelnon = &Fenetre::PasDeChoix;
		QuestionChoix( "Des logiciels ont déjà été démarrés :", 0 );
    }
    else
		DemarrerContexte();
}
void Fenetre::StopperTout()
{
    if(!demarreur.DesProcessusSontDemarres())
		MessageInfo("Arrêt des logiciels : il n'y a pas de logiciels démarrés");
    else
    {
		pFncAppelnon = nullptr;
		pFncAppeloui = &Fenetre::TuerProcess;
		QuestionOuiNon( "Si des fichiers ont été modifiés et n'ont pas été sauvegardés, les modifications seront perdues !!",
		"Continuer l'arrêt des logiciels ?");
    }
}
void Fenetre::AProposDe()
{
}
void Fenetre::ChoixDuContexte()
{
	if(session.pFichierProjet()->FichierExiste())
	{
		ChoisirContexte(0);
	}
	else
		MessageInfo( "La base de projets est vide. Créez au moins un projet." );
}
void Fenetre::NouveauProjet()
{
    session.SauverEnrgProjet();
	session.ReactionEnRetour(sigc::mem_fun(*this, &Fenetre::TraitementRetourContexte));
...
{
	CfgMTStartApp.ReactionEnRetour(sigc::mem_fun(*this, &Fenetre::TraitementRetourConfig));
    CfgMTStartApp.GestionConfiguration( 0 );
}
void Fenetre::CreerUneConfiguration()
{
	CfgMTStartApp.CreerUneNouvelleConfiguration( 0 );
	//pFncVoid = &Fenetre::CreerUneConfiguration; 
	//pFncRetourValeur = &Fenetre::Reponse_Entree; 
...
void Fenetre::FermerAppliContexte()
{
	Logiciel *pobjet = new Logiciel( &CfgMTStartApp, pscene, 0, 110 );
	if(!pobjet->okvalide)
		delete pobjet;
	else
...
/**	SLOTS MENU Graphique ( barre en haut ) **/
void Fenetre::ModifierPriorite()
{
    ...
    else
    {
		...
		AppliquerModificationPriorite( 0 );
    }
}
void Fenetre::LierDesObjets()
{
    ...
    if( pelem!=nullptr )
    {
		...
		{
			case 110 :
				MessageInfo("L'objet ne peut être attaché !");
				break;
			case 200 :
...
{
	//if( CfgMTStartApp.ConfigListeLogicielModifiee() )
	//{
		//MessageInfo( "La configuration liste de logiciel a été modifiée./n Le menu est mis à jour." );
		//TraitementRetourConfig();
	//}
	//else
	//{
		uint8_t mnu = cas/100;
...
/** Routines menus gestion **/
void Fenetre::DemarrerContexte()
{
	ImageAlternative(2,false);
	passortir    = true;
	switch (demarreur.DemarrerLaSession())
	{
	case 1 :
		close();
		break;
	case 2 :
		//messagemsa->PasDeLogicielsEnScene();
	default :
		passortir    = false;
...
void Fenetre::NettoyerLaScene(){ pscene->Nettoyer(); }
void Fenetre::close()
{
	EnregistrerContexte();
	session.Enregistrer();
...
void Fenetre::EcrireContexte()
{
	pFncAppeloui = nullptr;
	pFncAppelnon = nullptr;
	GraphiqueSurface* pgraphiquestop = nullptr;
	FBlocObjetContexte  ficobjetctx( session.CheminProjet() );
	lstobjettmp.clear();
	std::vector<GraphiqueSurface*> lstliaison;
	pscene->CollecterGenreObjet( 100, lstobjettmp );
	pgraphiquestop = pscene->CollecterGenreObjet( 110 );
	if(pgraphiquestop != nullptr)
		lstobjettmp.push_back(pgraphiquestop);
	pscene->CollecterGenreObjet( 5000, lstliaison );
	uint32_t i;
	for( i=0; i<lstobjettmp.size(); i++ )
	{
		ptrlogTMP = (Logiciel*)lstobjettmp.at(i);
	...
	for( i=0; i<lstliaison.size(); i++ )
	{
		  ...
		  )
		{
			if( psourcetmp->GenreElement() == pcibletmp->GenreElement() )
			{
				...
				for ( ctmp = lstobjettmp.begin() ; ctmp != lstobjettmp.end(); ++ctmp)
				{
					if( *ctmp==psourcetmp )
					{
						ficobjetctx.PtrEnrgLia()->ValeurSource(ctmp-lstobjettmp.begin());
						oktrouve++;
					}
					if( *ctmp==pcibletmp )
					{
						ficobjetctx.PtrEnrgLia()->ValeurCible(ctmp-lstobjettmp.begin());
						oktrouve++;
					}
				}
				if(oktrouve==2)
...
void Fenetre::TraitementRetourConfig( uint8_t codret )
{
	DGChaineListe	lstnomtmp;
	if( codret & 4 )
	{// liste logiciel modifiée
		lstobjettmp.clear();
		std::list<Gtk::Button*>::iterator itbtn;
		
		for( itbtn = lstbtnlogiciel.begin(); itbtn != lstbtnlogiciel.end(); ++itbtn )
		{
			phrtmp1 = (**itbtn).get_label();
			lstnomtmp.PtrChaine()->push_back( phrtmp1 );
			zonegauche->remove(**itbtn);
		}
		lstbtnlogiciel.clear();
		BarreBoutonLogiciel();
		for( itbtn = lstbtnlogiciel.begin(); itbtn != lstbtnlogiciel.end(); ++itbtn )
		{
			lstnomtmp.Effacer((**itbtn).get_label());
		}
		if( !lstnomtmp.NonVide() )
		{
			if(pscene->CollecterGenreObjet( 100, lstobjettmp ))
			{
				for( uint32_t i=0; i<lstobjettmp.size(); i++ )
				{
					ptrlogTMP = (Logiciel *)lstobjettmp.at(i);
					if( lstnomtmp.Trouver( ptrlogTMP->NomElement() ) )
						SupprimerObjetLogiciel();
				}
			}
		}
	...
	if( codret & 8 )
	{// liste priorite modifiée
		lstnomtmp.Vider();
		lstobjettmp.clear();
		if( CfgMTStartApp.ListeLabelPriorites( lstnomtmp ) )
		{
			if(pscene->CollecterGenreObjet( 100, lstobjettmp ))
			{
				for( uint32_t i=0; i<lstobjettmp.size(); i++ )
				{
					ptrlogTMP = (Logiciel *)lstobjettmp.at(i);
					if( !lstnomtmp.Trouver( ptrlogTMP->NomPriorite() ) )
					{
						ptrlogTMP->ReconfigurerGraphique( 0 );
						pscene->ActualiserGraphe( ptrlogTMP );
					}
				}
			}
		}
...
void Fenetre::TraitementRetourContexte()
{
	NettoyerLaScene();
	TitreDeLaFenetre();
	phrtmp1 = session.NomConfiguration();
	if(phrtmp1.compare(CfgMTStartApp.ConfigurationEnCours())!=0)
	{
		CfgMTStartApp.LireConfiguration(phrtmp1);
		std::list<Gtk::Button*>::iterator itbtn;
		for( itbtn = lstbtnlogiciel.begin(); itbtn != lstbtnlogiciel.end(); ++itbtn )
			zonegauche->remove(**itbtn);
		lstbtnlogiciel.clear();
...
void Fenetre::reponse_choix_demarrer( uint8_t vchoix )
{
	pFncAppelnon, pFncRetourValeur  = nullptr;
	switch(vchoix)
	{
		case 1 : TuerProcess();break;//1 stopper et démarrer
		case 2 : okdemarrer=false;
	}
	ViderListeChoix();
    if(okdemarrer)
		DemarrerContexte();
...
/** Routines menus graphique **/
void Fenetre::AppliquerModificationPriorite( uint8_t valrt )
{
	if( valrt == 0 )
	{
		...
		if( CfgMTStartApp.ListeLabelPriorites( lstinfo ))
		{
			...
			dlgchoixprio->ConnexionRetour(sigc::mem_fun(*this, &Fenetre::AppliquerModificationPriorite));
		}
	}
	else
	{
		if(dlgchoixprio!=nullptr)
		{
			...
			if( valrt==1 )
			{
				if( ptrlogTMP != nullptr )
				{
					ptrlogTMP->ReconfigurerGraphique( valeur );
					pscene->ActualiserGraphe( ptrlogTMP );
				}
			}
		}
...
{
	if(index<lstobjimage.size())
	{
		if(sens)
			std::tie ( ptrimagetmp, ptrimgpixbuftmp, std::ignore ) = lstobjimage.at(index);
		else
			std::tie ( ptrimagetmp, std::ignore, ptrimgpixbuftmp ) = lstobjimage.at(index);
		ptrimagetmp->set( ptrimgpixbuftmp );
	}
}
void Fenetre::TuerProcess()
{
...
void Fenetre::CreerLeMenu()
{
	...
	auto zonecentrale = Gtk::make_managed<Gtk::Box>( Gtk::Orientation::VERTICAL, 5 );
		...
		pscene->set_name("zonegraphe");
			...
			pscene->signal_resize().connect(sigc::mem_fun(*pscene, &ZoneGraphique::ChangerDimensionsScene)); 	
	...
	auto zonegauchebas = Gtk::make_managed<Gtk::Box>( Gtk::Orientation::VERTICAL, 2 );
		zonegauchebas->set_expand(false);
		zonegauchebas->set_halign(Gtk::Align::CENTER);
		zonegauchebas->set_valign(Gtk::Align::START);
		zonegauchebas->set_name("barreicon");
		
	barregauche->append(*zonegauche);
	barregauche->append(*zonegauchebas);
...
//    int hticodrt	= 40;
	...
	if( CreerBoutonImage( hticodrt, barredroitehaut ) )
		ptrboutontmp->signal_clicked().connect( sigc::mem_fun(*this, &Fenetre::AProposDe) );
	if(CreerBoutonImage( hticodrt, barredroitehaut ))
		ptrboutontmp->signal_clicked().connect( sigc::mem_fun(*this, &Fenetre::ChoixDuContexte));
	if(CreerBoutonImage(hticodrt, barredroitehaut ))
	...
	if(CreerBoutonImage( hticodrt, barredroitebas ))
		ptrboutontmp->signal_clicked().connect( sigc::mem_fun(*this, &Fenetre::CreerUneConfiguration) );
	if(CreerBoutonImage( hticodrt, barredroitebas ))
	...
	if( CreerBoutonImage( hticodrt, zonegauchebas ) )
		ptrboutontmp->signal_clicked().connect( sigc::mem_fun(*this, &Fenetre::FermerAppliContexte) );
...
**/
	...
	if(CreerBoutonImage( hticohaut, barrehaute ))
		ptrboutontmp->signal_clicked().connect( sigc::mem_fun(*this, &Fenetre::ModifierPriorite) );
	if(CreerBoutonImage( hticohaut, barrehaute ))
	...
	if(CreerBoutonImage( hticohaut, barrehaute ))
		ptrboutontmp->signal_clicked().connect( sigc::mem_fun(*pscene, &ZoneGraphique::CentrerSchema) );
/**
 * 
	...
	if((lignedonnee->read_line(phrtmp1))&&(lignedonnee->read_line(phrtmp2)))
	{
		...
		if(img_alternat)
		{
			...
			if(ptrimgpixbuftmp!=nullptr)
			{
				...
				ptrimgaltpixbuftmp = Gdk::Pixbuf::create_from_resource(phrtmp2);//resobjetactif_get_resource
				
				valimgbtnalt = std::make_tuple( ptrimagetmp, ptrimgpixbuftmp, ptrimgaltpixbuftmp );
				lstobjimage.push_back(valimgbtnalt);
			}
		}
		else
		{
...
void Fenetre::QuestionChoix( const Glib::ustring& libmess, const uint8_t &rtbtn )
{
	if( libmess.empty() )
	{
		if( messagemsa!=nullptr )
		{
			uint8_t valrt = messagemsa->Index();
			delete messagemsa;
			messagemsa=nullptr;
			if(rtbtn==1)
			{
	...
	else
	{
		...
		messagemsa->AppelChoix( this, libmess, plstchoixdialogue );
		messagemsa->connexionChoix(sigc::mem_fun(*this, &Fenetre::QuestionChoix));
	}
}
void Fenetre::SaisirValeur(const Glib::ustring& liblibel, const uint8_t &typezone )
{
	if(liblibel.empty())
	{
		if(bdialogue!=nullptr)
	...
	else
	{
		if(bdialogue!=nullptr) delete bdialogue;
		bdialogue = new MessageMSA;
		bdialogue->AppelEntree(this,liblibel,typezone);
		bdialogue->connexionChoix(sigc::mem_fun(*this, &Fenetre::SaisirValeur));
	}
}
